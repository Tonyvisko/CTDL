...
void LayDanhSachDauSach(pointerArray *&root){
	DauSach x;
	string ISBN;
	string TenSach;
	int SoTrang;
...
void ChuyenCayThanhMang_DauSach(pointerArray* root, DauSach* MangDauSach, int& n) {
    if (root != NULL) {
        ChuyenCayThanhMang_DauSach(root->left, MangDauSach, n);
        MangDauSach[n++] = root->data;
        ChuyenCayThanhMang_DauSach(root->right, MangDauSach, n);
    }
}
